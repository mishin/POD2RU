...
Это B<Не> определение API, это чисто внутреннее руководство для тех, кто хочет хачить
 (программировать) регекс движок, или хочет понять, как он работает. Читатели
этот документа, как ожидается, понимают синтаксис регулярных выражений perl и 
используют его в деталях. Если вы хотите узнать об основах регулярных выражений Perl
, см L<perlre>. И, если вы хотите заменить регекс движоксвоим собственным, см L<perlreapi>.
...
Когда мы говорим о регексах нам нужно различать их источник
код формы и их внутреннюю форму. В настоящем документе мы будем использовать термин
...
соответствует условиям,  I<S-регкс> и I<B-регкс>, что Марк Джесон
Доминус использует в своей статье о  "Rx"  ([1] в L</REFERENCES>).

=head2 Что такое движок регулярных выражений?

Движком регулярных выражений является программа, которая принимает набор ограничений
в мини-языке, а затем применяет эти ограничения на
целевую строку и определяет, удовлетворяет ли строка
этим ограничениям. Смотрите L<perlre> для полного определения этого языка.
...
И мы должны сделать все это эффективно.

=head2 Структура Ререксп программы

=head3 Верхний уровень

Хотя это немного запутывающее, и некоторые люди возражают против терминологии, но
...
конечного автомата (типа синтаксической диаграммы или or "железнодорожная нормальная форма" в технологии
 парсинга).>

Термин "железнодорожная нормальная форма" немного эзотеричен, с "синтаксисом схем/диаграмм", 
или "железнодорожных диаграмм/графиков", будучи более общих терминов.
...
статистически их не много), и весь образует макет с
...
для сравнения. Поскольку "узел" и "код операции" ("opcode") уже имеют другие значеиая в
исходниках Perl, мы будем называть узлы в программе регекс "регопеация" "регопс" "regops"а.

CПрограмма  представлена массивом структур C<regnode>, одна или
