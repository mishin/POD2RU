...
По состоянию на Perl 5.9.5 есть новый интерфейс для подключения и использования
движка регулярных выражений другого, чем по умолчанию.

Каждый движок должен предоставлять доступ к постоянной структуре
следующий формат:
