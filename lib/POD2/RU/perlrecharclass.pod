...
Документацию верхнего уровня о регулярных выражениях Perl можно найти в L<perlre>.

Это руководство обсуждает синтаксис и использование символьных 
классов регулярных выражений Perl.

Класс символов — это способ обозначения набора символов
таким образом, что будет найден один символ из набора.
Важно помнить, что: сопоставление класса символов
соответствует ровно одному символу в строке источника. (Строка источник
 является строкой с которой сравнивается регулярное выражение.)

Существует три типа символьных классов в регулярных
выражениях Perl: точка, бекслеш последовательности (backslash sequences) и формы, заключенные в квадратные 
скобки. Имейте в виду, однако, что часто использование термина «символьный класс»
подразумевает форму в квадратных скаобках. Конечно большинство документации Perl делает это.

=head2 Точка

Точка (или период),C<.>  является вероятно наиболее часто используемой и конечно же
наиболее известным классом символов. По умолчанию точка соответствует любому
символу, за исключением новой строки. Это значение по умолчанию можно изменить
добавлением сопоставление строки с помощью модификатора I<единая срока>  I<single line>:
 либо для всего регулярного выражения модификатором C</s> , или
локально с помощью C<(?s)>. ( Последовательности обратной косой черты C<\N>, описанные
ниже, соответствует любому символу, за исключением новой строки без связи с
модификатором  I<единая срока>.

Вот несколько примеров:

 "a"  =~  /./       # найдёт
 "."  =~  /./       # найдёт
 ""   =~  /./       # не найдёт (точка находит 1 символ)
 "\n" =~  /./       # не найдёт (точка не соответствует символу новой строки)
 "\n" =~  /./s      # найдёт (глобальный модификатор 'единая строка' ('single line'))
 "\n" =~  /(?s:.)/  # найдёт (локальный модификатор 'единая строка')
 "ab" =~  /^.$/     # не найдёт (точка находит 1 символ)

=head2 Последовательности обратной косой черты (backslash)
X<\w> X<\W> X<\s> X<\S> X<\d> X<\D> X<\p> X<\P>
X<\N> X<\v> X<\V> X<\h> X<\H>
X<word> X<whitespace>

Последовательности обратной косой черты (backslash sequences) это последоветельности
символов, где первой стоит обратная косая черта.  
Perl придает особое значение для многих таких последовательностей и некоторые из них
являются классами символов. То есть они соответствуют каждый одному символу,
при том условии, что символ принадлежит конкретному набору символов, определенных
как последовательность.

Вот список последовательностей обратной косой черты, которые являются классами символов. Они
обсуждаются более подробно ниже. (Для последовательностей обратной косой черты, которые не являются
классами символов, см L<perlrebackslash>).

 \d             Найдет символ десятичной цифры.
 \D             Найдет символ недесятичной цифры.
 \w             Найдет "словесный" символ.
 \W             Найдет "не-словесный" символ.
 \s             Найдет пробельный символ.
 \S             Найдет не-пробельный символ.
 \h             Найдет горизонтальный пробел
 \H             Найдет Негоризонтальный пробел.
 \v             Найдет вертикальный пробел.
 \V             Найдет не-вертикальный пробел.
 \N             Не новая строка.
 \pP, \p{Prop}  Найдет символ с Юникодным свойством
 \PP, \P{Prop}  Найдет символ с отсутсвующим Юникодным свойством
